...
import { isFarmer } from '../../auth/utils/roles';
...
export default function HomeScreen({ navigation }) {
  ...
  const farmerMode = isFarmer(userRole);
  ...
  const farmerSections = [
    {
      ...
      items: [
        { id: 'myTasks', title: 'Công việc của tôi', icon: 'check-circle', route: 'MyTasks', color: '#2563eb', iconBg: '#eff6ff' },
        { id: 'journals', title: 'Nhật ký canh tác', icon: 'book-open', route: 'Journals', color: '#16a34a', iconBg: '#dcfce7', badge: String(normalizedStats.totalJournals) },
        { id: 'productionPlans', title: 'Kế hoạch sản xuất', icon: 'calendar', route: 'ProductionPlans', color: '#7c3aed', iconBg: '#f5f3ff' },
        { id: 'productBatches', title: 'Lô sản phẩm', icon: 'box', route: 'ProductBatches', color: '#ea580c', iconBg: '#fff7ed' },
      ],
    },
    {
      title: 'Hỗ trợ và thông tin',
      items: [
        { id: 'notifications', title: 'Thông báo', icon: 'bell', route: 'Notifications', color: '#f59e0b', iconBg: '#fff7ed', badge: unreadCount > 0 ? (unreadCount > 99 ? '99+' : String(unreadCount)) : '' },
        { id: 'trace', title: 'Truy xuất QR', icon: 'grid', route: 'Scanner', color: '#16a34a', iconBg: '#ddfbea' },
      ],
    },
  ];

  const visibleSections = farmerMode ? farmerSections : sections;
