  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity style={styles.headerButton} onPress={() => navigation.goBack()} accessibilityLabel="Quay lại"><Feather name="arrow-left" size={24} color="#fff" /></TouchableOpacity>
        <Text style={styles.headerTitle}>Chi tiết thông báo</Text>
        <View style={styles.headerButton} />
      </View>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.card}>
          <View style={styles.iconWrap}><Feather name="bell" size={28} color="#15803d" /></View>
          <View style={styles.titleRow}><Text style={styles.title}>{notification.title || 'Thông báo'}</Text>{unread ? <View style={styles.newBadge}><Text style={styles.newBadgeText}>Mới</Text></View> : null}</View>
          <View style={styles.dateRow}><Feather name="clock" size={15} color="#64748b" /><Text style={styles.date}>{formatVietnamDateTime(notification.createdAt || notification.sentAt, 'Không xác định')}</Text></View>
