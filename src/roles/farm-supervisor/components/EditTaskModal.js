...
export default function EditTaskModal({ visible, task, users = [], onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (visible && task) {
      setTaskName(task.name || task.taskName || '');
      setDescription(valueOf(task.description, task.notes, task.content, ''));
      setActivityType(task.activityType || 'OTHER');

      const initialLeader = valueOf(
        task.assignedLeaderId, task.leaderId,
        task.assignedLeader?.id, task.leader?.id
      );
      setLeaderId(initialLeader || null);

      const rawFarmerIds = task.farmerIds || task.assignedFarmerIds || (task.assignments || []).map((a) => a.userId || a.farmerId || a.id);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await supervisorApi.updateTask(taskId, payload);
      setSaving(false);
      Alert.alert('Thành công 🎉', 'Cập nhật thông tin công việc thành công!', [
        { text: 'OK', onPress: () => onSuccess?.() },
      ]);
    } catch (err) {
      setSaving(false);
      const msg = getApiErrorMessage(err, 'Không thể cập nhật công việc.');
      Alert.alert('Lỗi cập nhật ⚠️', msg);
    }
  };
        ...
        <View style={styles.card}>
          ...
          <View style={styles.header}>
            <View style={styles.headerTitleRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.headerTitle}>Sửa thông tin công việc</Text>
          ...
          <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            {/* Assign Farmers */}
            <View style={styles.inputGroup}>
              <Text style={styles.label}>Phân công Nông dân ({farmerIds.length} đã chọn)</Text>
